...
```
pack build python-function-with-git-3 --path . --builder ghcr.io/vmware-tanzu/function-buildpacks-for-knative/functions-builder:0.3.1 --env BP_FUNCTION=func.main
```

Where `python-function-with-git-3` is the name of your runnable function image.

Then run via Docker:

```
docker run -it --rm -p 8080:8080 python-function-with-git-3
```
...
```
tanzu apps workload create python-function-with-git-3 -f config/workload.yaml \
  --local-path . \
  --source-image <REPOSITORY-PREFIX>/python-function-with-git-3-source \
  --type web
```
...
```
tanzu apps workload get python-function-with-git-3
```
